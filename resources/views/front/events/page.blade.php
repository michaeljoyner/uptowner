...
@section('content')
    ...
    <section class="pd-y-7 flex-col flex-center-y">
        ...
        <p class="text-constrain body-text text-large text-center">The Uptowner is one of the few places in Taichung
    ...
    <section class="pd-y-7">
        ...
        @foreach($events->restOfEvents() as $event)
            <div class="pd-4 w-con-800 mg-x-a">
                <p class="mg-y-2 text-colour h4">@include('svgicons.calendar') {{ $event->event_date->format('jS M, Y') }}</p>
                <p class="h3">@include('svgicons.shield') {{ $event->name }}</p>
                <p class="pl-7 mg-y-2 body-text text-largish">{{ $event->description }}</p>
                <p class="mg-y-2 body-text">@include('svgicons.watch') {{ $event->time_of_day }} </p>
            </div>
        @endforeach
    </section>
@endsection
